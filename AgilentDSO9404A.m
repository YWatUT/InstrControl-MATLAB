classdef AgilentDSO9404A
    % Agilent 4 GHz scope borrowed from Prof. Jia's group
    ...
    %% Methods
    methods
        function obj = AgilentDSO9404A(instr_address)
            % Open the instrument at the given address
            try
                % need to change these lines
                obj.instr_handle = visa('ni',instr_address);
                obj.instr_handle.InputBufferSize = 1000000;
                fopen(obj.instr_handle);
                disp('Agilent DSO9404A connected successfully');
            catch ME
                disp(ME.message);
            end
        end
